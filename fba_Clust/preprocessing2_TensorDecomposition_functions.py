...
def multipleTensorDecomposition(paths,dir,ICA_list):
    for p in paths:
        tensor = np.load(p)
        ICA_comp = re.search(r'\d+', p).group()
        # Compute tensor decomposition only on the tensor(s) with the ICA components that are in ICA_list
        if int(ICA_comp) in ICA_list:
