...
def computeK(matrix,kmax):
    ...
    # similarity within the clusters
    fig, ax = plt.subplots()
...
### identity for multiple paths ###

# definition of the function that get all the paths of the .npy files
def getPaths(directory):
    p = []
    # browse every files in the directory
    for filename in os.listdir(directory):
        # only get the files that starts with "patientsMatrix"
        if filename.startswith("patientsMatrix"):
...
def identityMultiplePaths(paths,metaboGrps):
    perc_clusters = []
    comps = []
    # browse each numpy files in the directory
    for p in paths:
        patientsMatrix = np.load(p)
        c = re.search(r'\d+', p).group()
        comps.append(c)

        clusters = getCl_K(patientsMatrix, 4)
        best_perScl, best_permutation, best_clusters = getIdentityPerC(clusters, metaboGrps)
        print('Tensor with',c,'ICA components :',best_perScl,'% of identity with the metabolic groups')
        perc_clusters.append(best_perScl)

    # plot the percentage of identity per ICA components
    fig, ax = plt.subplots()
    plt.scatter(comps, perc_clusters)
    plt.xlabel('ICA components')
    plt.ylabel('Identity (%)')
    # save the plot as svg file
    fig.savefig('identityMultipleComponents.svg', format='svg')

    # get the ICA component(s) with the highest percentage of identity
    indices = [i for i, x in enumerate(perc_clusters) if x == max(perc_clusters)]
    maxComps = [comps[i] for i in indices]

    print('The percentage of identity is the highest (',perc_clusters[indices[0]],'% ) for', maxComps, 'ICA component(s)')
