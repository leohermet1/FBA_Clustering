...
def identityMultiplePaths(paths,metaboGrps):
    ...
    for patientsMatrix in paths:
        tensor = np.load(p)
        comps.append(re.search(r'\d+', patientsMatrix).group())

        clusters = getCl_KpatientsMatrix, 4)
        ...
        print('The percentage of identity is the highest and equal to',perc_clusters[indices[0]],'% for', maxComps, 'ICA component(s)')
