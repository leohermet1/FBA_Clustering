...
parser.add_argument('pathPatientsMatrix', help= 'Input(str): Path of the patient matrix (from the preprocessing run) as .npy file OR directory with all the patient matrices if using -Identity')
...
parser.add_argument('-Identity', action='store_true', help= 'CANNOT BE USED WITH OTHER OPTIONS, Compute the percentage of identity of multiple patient matrices between the 4 computed clusters and the metabolic groups')
...
if args.Identity:
    # Get the paths of the patient matrices (output of the tensor decomposition)
    npyDir = args.pathPatientsMatrix
    paths = v.getPaths(npyDir)
    # Compute their percentage of identity with metabolic groups
    v.identityMultiplePaths(paths,metaboGrps)
    ...
    if args.cl:
        ...
        v.saveIMG(Xt, v1, v2, 'PCA of the patients with computed clusters', 'PCA_patients_Kcl.svg', cl=True,
                  clusters=clusters, cl_name='clusters')

    if args.Mgrp:
        # Save the distribution with the metabolic groups
        v.saveIMG(Xt, v1, v2, 'PCA of the patients with metabolo groups', 'PCA_patients_Mgrp.svg', cl=True,
                  clusters=metaboGrps, cl_name='metabo grps')
